/*
 ...
 @Component Name: ladderTable.drv
...
import './ladder-table.less';

export default angular.module('app.section.ladderTable', ['datatables', 'datatables.buttons', 'datatables.columnfilter'])
    .directive('ladderTable', ladderTableConfig);

function ladderTableConfig() {
    return {
        restrict: 'E',
        replace: true,
        scope: {
        ...
        template: require('./ladder-table.tpl.html'),
        controller: LadderTableController,
        controllerAs: 'ladderTable'
    }
}

class LadderTableController {
    constructor($scope, DTOptionsBuilder, DTColumnBuilder, $timeout, originService, $q, $http, $compile) {
        ...
        this.ladderTableData = $scope.data;
        this.instance = {};
        this.checkboxs = {};
        this.dataTable = $http.post(`${originService.apiUrl}/api/layer/getLadder`);
        ...
        this.dtColumns = [
            ...
            DTColumnBuilder.newColumn('End Rate').withTitle('End Rate')
        ];
    }
    ...
    createFilters() {
        ...
        angular.forEach(filtersArr, (value, key) => {
            ...
            elements += `<label><input type="checkbox" id="${value}" class="filter" ng-model="ladderTable.checkboxs.${value}" ng-click="ladderTable.updateTable($event)">${value}</label>`;
        });
        elements += `</div>`;
        let filters = $(`.${this.ladderTableData.class} .dataTables_filter`).append(this.$compile(elements)(this.$scope));
        this.$timeout(() => {
    ...
    updateTable(event) {
        let filterRules = [];
        let dateFilter = this.ladderTableData.dateFilter.id.split(',');
    ...
    //addClassStatus() {
    //    $(`.${this.ladderTableData.class} tbody tr[role=row]`)
    ...
    //onSearch() {
    //    let self = this;
    //    var elem = $(`.${this.ladderTableData.class} input[type=search]`);
    //
    ...
    //onReorder() {
    //    let self = this;
    //    $(`.${this.ladderTableData.class} .paginate_button, thead tr[role=row]`).click(function() {
    //        setTimeout(() => {
    //            self.addClassStatus();
    //            self.onReorder();
    //        }, 0);
    ...
    searchInit() {
        let input = $(`.${this.ladderTableData.class} input[type=search]`);
        $(`.${this.ladderTableData.class} .dataTables_filter`).empty().append(input);
        $(`.${this.ladderTableData.class} input[type=search]`).attr('placeholder', 'Search...');
    }
    ...
    addDateFilter() {
        this.ladderTableData.dateFilterArr = [];
        ...
        let select = `<select
            class="dateFilter" ng-model="ladderTable.ladderTableData.dateFilter"
            ng-options="option as option.label for option in ::ladderTable.ladderTableData.dateFilterArr track by option.id"
            ng-change="::ladderTable.updateTable()">`;
        let year = new Date().getFullYear();
        let month = new Date().getMonth()+1;
        let day = new Date().getDate();
        let today = new Date();
        this.ladderTableData.dateFilterArr.push({
        ...
        this.ladderTableData.dateFilter = this.ladderTableData.dateFilterArr[0];
        $(`.${this.ladderTableData.class} .dataTables_filter`).append(this.$compile(select)(this.$scope));
        this.$scope.$digest();
        this.$timeout(() => {
            this.updateTable(this.dataTable);
        });
