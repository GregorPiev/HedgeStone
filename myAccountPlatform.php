...
<script type="text/javascript">
    ...
    $(document).bind('spotoptionPlugin.ready', function () {
        ...
        $.each($('.formTitle'), function( index, value ) {
          $(value).hide()
        });

        $('.navigationSidebar').hide();

        $('.allZonesContainer').css('float', 'left');
        $('.allZonesContainer').css('border', '0');


        // make sure we're in the position's tab. If not - display error
        var positionTab = $('.navTab_positions')[0];
        if(positionTab.length < 1) {
            $('.allZonesContainer').html('<div style="color: white; font-size: 14px; background-color: transparent;">Not Available</div>')
        } else {
            positionTab.click();
        }
    });
